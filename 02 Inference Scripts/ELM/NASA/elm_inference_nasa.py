# -*- coding: utf-8 -*-
"""ELM Inference NASA.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/10v9tUK81mOAW0JzMrSzrDUvIe16AY-VB
...
# Load test tensors
X_test, y_test, test_sequence, test_labels = torch.load('test_tensors_ELM_NASA.pt', map_location=torch.device(device))

# Load model
model = {}
for leave_out in Battery_list:
  model[leave_out] = torch.load(f'checkpoints/ELM_NASA_{leave_out}.pt', map_location=torch.device(device))
...
for leave_out, idx in zip(Battery_list, range(len(Battery_list))):
  ...
  print(f"Accuracy on {leave_out}: {acc*100}% | MAE: {mae:.5f} | RMSE: {rmse:.5f}")
...
plt.savefig('One-step_ELM_NASA.pdf')

import pandas as pd
# Convert to PD DataFrame
OSP_results = pd.DataFrame([OSP_ELM_NASA['B0005'],
                            OSP_ELM_NASA['B0006'],
                            OSP_ELM_NASA['B0007'],
                            OSP_ELM_NASA['B0018']
                            ])
# Saving results
OSP_results.to_pickle("OSP_ELM_NASA.pkl")
...
predictions = {}
plt.figure(figsize=(40,7))
plt.suptitle(f'ELM (NASA) Multi-Step Ahead', fontsize=16, weight='bold')
...
plt.savefig('Multi-step_ELM_NASA.pdf')

# Convert to PD DataFrame
MSP_results = pd.DataFrame([MSP_ELM_NASA['B0005'],
                            MSP_ELM_NASA['B0006'],
                            MSP_ELM_NASA['B0007'],
                            MSP_ELM_NASA['B0018']
                            ])
